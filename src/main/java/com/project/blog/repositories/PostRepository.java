...
//    @Query("SELECT com.project.blog.dtos.PostWithCommentsDTO(p.id, p.title, p.content, p.dateCreated, p.dateEdited, c) FROM Post p INNER JOIN Comment c ON p.id = c.post_id WHERE p.id = :pid")
//    PostWithCommentsDTO getPostWithComment(@Param("pid") Long pid);
}
